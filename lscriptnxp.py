    ...
    def from_mapping(cls, payload: dict[str, Any]) -> "AppConfig":
        ...
        return cls(
            ...
            server=ServerConfig(
                ...
                key_pub_path=str(server_cfg["key_pub_path"]) if server_cfg.get("key_pub_path") else None,
            ),
            dut=DutConfig(
...
def resolve_provision_args(config: AppConfig, args: argparse.Namespace, config_path: pathlib.Path) -> ProvisionArgs:
    ...
    with open_db_connection(config_path, config.db) as connection:
        ...
        if row is not None:
            ...
            base_mac = normalize_mac(new_macs[0])
            switch_mac = normalize_mac(new_macs[1])
    pub_key_raw = args.pub_key or config.server.key_pub_path
    if not pub_key_raw:
        raise ConfigError("Missing public key path. Provide --pub-key or set server.key_pub_path in the YAML.")
    pub_key_path = pathlib.Path(pub_key_raw).expanduser()
    if not pub_key_path.is_file():
        raise FileNotFoundError(f"Public key file not found: {pub_key_path}")
    private_key_path = pathlib.Path(args.private_key).expanduser()
    return ProvisionArgs(
        ...
        pub_key_name=pub_key_path.name,
        pub_key_path=pub_key_path,
        private_key_path=private_key_path,
        pub_key_text=pub_key_path.read_text(encoding="utf-8").strip(),
    )
...
def stage_network_in_emergency(session: SerialSession, config: AppConfig) -> None:
    ...
    output = run_command_capture(
        ...
        "Configure DUT IP and ping the PC from emergency mode",
    )
...
def install_authorized_key_file(session: SerialSession, prompt_pattern: re.Pattern[str], pub_key_name: str, remote_dir: str) -> None:
    run_command(session, f"mkdir -p {remote_dir}", prompt_pattern, 30, f"Create {remote_dir}")
    run_command(session, f"chmod 700 {remote_dir}", prompt_pattern, 30, f"Set {remote_dir} permissions")
    run_command(session, f"touch {remote_dir}/authorized_keys", prompt_pattern, 30, "Create authorized_keys")
    run_command(session, f"cat {remote_dir}/{pub_key_name} >> {remote_dir}/authorized_keys", prompt_pattern, 30, "Append public key to authorized_keys")
    run_command(session, f"chmod 600 {remote_dir}/authorized_keys", prompt_pattern, 30, "Set authorized_keys permissions")
...
def upload_deploy_script_from_windows_to_dut(config: AppConfig, provision: ProvisionArgs) -> None:
    if paramiko is None:
        raise RuntimeError("paramiko is required for the Windows-to-DUT upload stage.")
    ...
    remote_path = f"{remote_dir}/{provision.deploy_script}"
    client = connect_over_ssh_password(config)
    try:
        ssh_exec_checked(client, f"mkdir -p {remote_dir}")
        ...
        if remote_size != local_size:
            raise RuntimeError(
                f"Uploaded size mismatch for {provision.deploy_script}: local={local_size}, remote={remote_size}"
            )
    finally:
        client.close()
...
def transfer_deploy_script_in_emergency(session: SerialSession, config: AppConfig, provision: ProvisionArgs) -> None:
    ...
    ls_output = run_command_capture(
        session,
        f"cd /{config.dut.tmp_path} && ls -all",
        emergency_prompt,
    ...
    if provision.deploy_script not in ls_output:
        raise RuntimeError(
            f"Expected {provision.deploy_script} in /{config.dut.tmp_path} after SCP transfer, but it was not listed."
        )
...
def wait_for_deploy_completion(session: SerialSession, config: AppConfig, deploy_start_pos: int) -> None:
    ...
    session.wait_for_pattern(
        ...
        label="deploy completion at emergency shell",
        start_pos=deploy_start_pos,
    )
...
def run_deploy_script(session: SerialSession, config: AppConfig, provision: ProvisionArgs) -> None:
    ...
    run_command(session, f"chmod +x /{config.dut.tmp_path}/{provision.deploy_script}", emergency_prompt, 30, "Make deployment script executable")
...
def save_ip_and_install_key_over_ssh(config: AppConfig, provision: ProvisionArgs, ssh_log_path: pathlib.Path) -> None:
    if paramiko is None:
        raise RuntimeError("paramiko is required for the SSH stages.")
    client = connect_over_ssh_password(config)
    try:
        ...
        ssh_exec_checked(client, "mkdir -p /root/.ssh && chmod 700 /root/.ssh && touch /root/.ssh/authorized_keys")
        with client.open_sftp() as sftp:
            remote_pub_path = f"/root/.ssh/{provision.pub_key_name}"
            with sftp.file(remote_pub_path, "w") as remote_file:
                remote_file.write(provision.pub_key_text + "\n")
            sftp.chmod(remote_pub_path, 0o600)
        escaped_pub_key = provision.pub_key_text.replace("'", "'\"'\"'")
        ssh_exec_checked(
            client,
            "grep -qxF "
            f"'{escaped_pub_key}' /root/.ssh/authorized_keys || echo '{escaped_pub_key}' >> /root/.ssh/authorized_keys",
        )
        ssh_exec_checked(client, "chmod 600 /root/.ssh/authorized_keys")
...
def reboot_after_ip_save_over_ssh(config: AppConfig) -> None:
    client = connect_over_ssh_password(config)
    try:
        try:
            client.exec_command("reboot", timeout=30)
        except Exception:
            pass
    finally:
        client.close()
...
def load_private_key(private_key_path: pathlib.Path):
    if paramiko is None:
        raise RuntimeError("paramiko is not installed.")
    if not private_key_path.is_file():
        raise FileNotFoundError(
            f"Private key file not found: {private_key_path}. "
            "The script uploaded the public key for the DUT, but Paramiko still needs the matching private key locally."
        )
    key_loaders = [
    ...
    for loader in key_loaders:
        try:
            return loader(str(private_key_path))
        except Exception as exc:  # noqa: BLE001
            last_error = exc
    raise RuntimeError(f"Could not load private key {private_key_path}: {last_error}")
...
def connect_over_ssh_with_key(config: AppConfig, provision: ProvisionArgs, ssh_log_path: pathlib.Path) -> None:
    if paramiko is None:
        raise RuntimeError("paramiko is required for the SSH handoff stage.")
    key = load_private_key(provision.private_key_path)
    wait_for_ssh(config.dut.final_ip, 22, max(config.timeouts.emergency_boot_seconds, 180))
    client = paramiko.SSHClient()
    client.set_missing_host_key_policy(paramiko.AutoAddPolicy())
    try:
        ...
        stdin, stdout, stderr = client.exec_command("hostname ; ip addr show dev eth0", timeout=60)
        output = stdout.read().decode("utf-8", errors="replace")
        error = stderr.read().decode("utf-8", errors="replace")
        ssh_log_path.write_text(output + ("\n[STDERR]\n" + error if error else ""), encoding="utf-8")
    finally:
        client.close()
...
def run_nxp_flow(config: AppConfig, provision: ProvisionArgs, args: argparse.Namespace, run_dir: pathlib.Path) -> int:
    ...
    ssh_log = timestamped_log_path(run_dir, "ssh")
    ...
    try:
        with SerialSession(
            ...
            stage(6, "Log in over SSH, save the DUT IP, and install the public key")
            save_ip_and_install_key_over_ssh(config, provision, ssh_log)

            stage(7, "Reset NXP after saving the persistent IP over SSH")
            reboot_after_ip_save_over_ssh(config)

            stage(8, "Switch NXP control to key-based SSH with Paramiko")
            connect_over_ssh_with_key(config, provision, ssh_log)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument(
        "--pub-key",
        default=None,
        help="Public key file copied to the DUT and appended to /root/.ssh/authorized_keys.",
    )
    parser.add_argument(
        "--private-key",
        default=r"C:\Images\id_ed25519_lsbb",
        help="Matching private key file used locally by Paramiko after the final reboot.",
    )
    parser.add_argument(
        "--show-uart",
