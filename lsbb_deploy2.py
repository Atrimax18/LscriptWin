...
class DeployGui(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("LSBB Deployment ver2.0")
    ...
    def _build_ui(self) -> None:
        ...
        for index, label in enumerate(DEPLOYMENTS):
            ...
            button.grid(row=0, column=index, sticky="ew", padx=4, ipady=8)
    ...
    def _refresh_buttons_from_db(self) -> None:
        ...
        if serial:
            try:
                allowed = allowed_deployments_for_row(read_deployment_row(serial))
                self.status_var.set("Ready")
            except Exception as exc:
                allowed = set()
