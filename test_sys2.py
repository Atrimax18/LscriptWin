        ...
        with serial.Serial(port=settings["port"], baudrate=settings["baudrate"], timeout=0.2, write_timeout=1) as uart:
            ...
            if prbs7_interfaces:
                shell_output = run_switch_uart_command(
                    uart,
                    setup,
                    "sudo config vlan member del 100 PortChannel1",
                    shell_output,
                )
                for interface in prbs7_interfaces:
                    shell_output = run_switch_uart_command(
                        uart,
                        setup,
                        f"sudo config portchannel member del PortChannel1 Ethernet{interface.split('/', 1)[1]}",
                        shell_output,
                    )
            ...
            shell_output = run_switch_uart_command(
                uart,
                setup,
                "sudo config portchannel member add PortChannel1 Ethernet10",
                shell_output,
            )
            shell_output = run_switch_uart_command(
                uart,
