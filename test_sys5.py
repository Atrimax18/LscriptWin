...
REPORT_ORDER = {
    ...
    "tests.fpga.mdm0_int_loopback": 1,
    "tests.fpga.mdm1_int_loopback": 2,
    "tests.fpga.mdm2_int_loopback": 3,
    "tests.fpga.mdm3_int_loopback": 4,
...
def report_label(name: str) -> str:
    ...
    if name.startswith("tests.asc_voltage."):
        return "ASC_" + name.rsplit(".", 1)[-1].upper()
    ...
    if name == "tests.fpga.uplink.snr_min":
        return "UPLINK_SNR"
    if name.startswith("tests.fpga.mdm") and name.endswith("_int_loopback"):
        return name.removeprefix("tests.fpga.").removesuffix("_int_loopback").upper() + "_INT_LOOPBACK"
...
def parse_output(output: str) -> dict[str, Any]:
    ...
    for name, measurement in parse_asc_read_volt_all(text).items():
        data[f"tests.asc_voltage.{name}"] = measurement.value
    ...
    for sx_id, state in re.findall(r"\b(SX[12])\s+-\s+Reset\s+Done,\s+Reset\s+State\s*=\s*([A-Z_]+)", text, re.IGNORECASE):
        data[f"tests.sx4000.{sx_id.lower()}.reset"] = "Reset Done"
        data[f"tests.sx4000.{sx_id.lower()}.state"] = state
    ...
    mdm_domain_re = re.compile(
        r"FPGA\s+JESD\s+Config\s+-\s+(MDM\d+)\s*:.*?"
        r"FPGA_MB_LOCK_status\s*=\s*(0x[0-9a-fA-F]+),\s*"
        r"FPGA_SH_LOCK_status\s*=\s*(0x[0-9a-fA-F]+),\s*"
        r"SX_EMB_LOCK_status\s*=\s*(0x[0-9a-fA-F]+),\s*"
        r"SX_SH_LOCK_status\s*=\s*(0x[0-9a-fA-F]+)",
    ...
    for domain, fpga_mb, fpga_mb_sh, sx_emb, sx_sh in mdm_domain_re.findall(text):
        data[f"tests.domain.{domain.lower()}.fpga_mb_lock_status"] = fpga_mb
        data[f"tests.domain.{domain.lower()}.fpga_mb_sh_lock_status"] = fpga_mb_sh
        data[f"tests.domain.{domain.lower()}.sx_emb_lock_status"] = sx_emb
        data[f"tests.domain.{domain.lower()}.sx_sh_lock_status"] = sx_sh
    ...
    int_loopback_match = parse_loopback_snr_group(
        data,
        text,
        r"(?:Int|Dig)\s+Loopback\s+Test\s*"
    ...
    parse_loopback_snr_group_after(
        ...
        r"\(\s*SNRs\s*=\s*([^)]+?)\s*\)\s*-\s*(Pass|Fail(?:ed)?)",
        "full_loopback",
        int_loopback_match.end() if int_loopback_match else 0,
    )

    for modem, snr, status in re.findall(
        r"MDM(\d+)\s*<->\s*FPGA\s+(?:Int|Dig)\s+Loopback\s+Test,\s*SNR\s*=\s*(-?\d+(?:\.\d+)?)\s*-\s*(Pass|Fail)",
        text,
        re.IGNORECASE,
    ):
        data[f"tests.fpga.mdm{modem}_int_loopback"] = float(snr)
        data[f"tests.fpga.mdm{modem}_int_loopback_status"] = status.lower()
...
def compare(
    ...
    if isinstance(tests_cfg, dict):
        ...
        if isinstance(fpga_cfg, dict):
            if include_loopback_tests:
                for modem in range(4):
                    int_key = f"mdm{modem}_int_loopback"
                    dig_key = f"mdm{modem}_dig_loopback"
                    loopback_key = int_key if int_key in fpga_cfg else dig_key
                    loopback_limits = fpga_cfg.get(loopback_key)
                    if isinstance(loopback_limits, dict) and ("min" in loopback_limits or "max" in loopback_limits):
                        loopback_actual = actual.get(f"tests.fpga.{loopback_key}")
                        loopback_status = actual.get(f"tests.fpga.{loopback_key}_status")
                        loopback_passed = loopback_actual is not None and loopback_status == "pass" and range_check(loopback_limits, loopback_actual)
                        loopback_display = loopback_actual if loopback_status in (None, "pass") else f"{report_value(loopback_actual)} ({loopback_status})"
                        results.append(CheckResult(f"tests.fpga.{loopback_key}", f"{loopback_limits.get('min')}..{loopback_limits.get('max')}", loopback_display, loopback_passed))
        ...
        asc_cfg = expected.get("asc_voltage_test")
        if not isinstance(asc_cfg, dict):
            asc_cfg = tests_cfg.get("asc_voltage", {})
        if isinstance(asc_cfg, dict):
            for rail_name, rail_limits in asc_cfg.items():
                if isinstance(rail_limits, dict) and ("min" in rail_limits or "max" in rail_limits):
                    add_range_check(results, f"tests.asc_voltage.{rail_name}", rail_limits, actual.get(f"tests.asc_voltage.{rail_name}"))
...
def run_paramiko_interactive(args: argparse.Namespace, command: str, setup: dict[str, Any] | None = None) -> str:
    ...
    sent_asc_voltage_commands = False
    started = time.monotonic()
        ...
        while True:
            ...
            if channel.recv_ready():
                ...
                if full_like_mode and not sent_full_test_command and RUN_SH_DONE_RE.search(clean_output) and ">>>" in clean_output:
                    if not sent_asc_voltage_commands:
                        print("\n[INFO] run.sh completed; reading ASC voltage monitors.")
                        send_nxp_python_command(channel, args, output_parts, ASC_CFG_COMMAND)
                        time.sleep(1)
                        read_output = send_nxp_python_command(channel, args, output_parts, ASC_READ_COMMAND)
                        ...
                        sent_asc_voltage_commands = True
...
def csv_units(name: str) -> str:
    ...
    if name == "tests.fpga.uplink.snr_min":
        return "dBm"
    if name.startswith("tests.fpga.mdm") and (name.endswith("_int_loopback") or name.endswith("_dig_loopback") or name.endswith("_full_loopback")):
        return "dB"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Run LSBB system tests over SSH and compare the output, including ASC voltages, with test_setup.yaml.")
