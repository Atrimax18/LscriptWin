...
WINDOWS_PING_LOSS_RE = re.compile(r"\(\s*(\d+(?:\.\d+)?)%\s*loss\s*\)", re.IGNORECASE)
POSIX_PING_LOSS_RE = re.compile(r"\b(\d+(?:\.\d+)?)%\s+packet loss\b", re.IGNORECASE)
MAX_PASSING_PACKET_LOSS_PERCENT = 50.0
...
def ping_loss_percent(output: str) -> float | None:
    match = WINDOWS_PING_LOSS_RE.search(output) or POSIX_PING_LOSS_RE.search(output)
    if not match:
        return None
    try:
...
def ping_success(output: str) -> bool:
    loss_percent = ping_loss_percent(output)
    return loss_percent is not None and loss_percent <= MAX_PASSING_PACKET_LOSS_PERCENT
...
def run_pc_ping_for_target(sfp_name: str, target: str, ping_count: int, ping_timeout: int) -> tuple[str, str]:
    ...
    passed = ping_success(ping_output)
...
def build_parser() -> argparse.ArgumentParser:
    ...
    parser.add_argument("--ping-count", type=int, default=4, help="Number of ICMP echo requests per data IP target.")
