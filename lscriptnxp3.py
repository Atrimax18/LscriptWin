...
def run_dir_for_dig_sn(dig_sn: str, suffix: str = "") -> pathlib.Path:
    folder = sanitize_log_folder_name(dig_sn)
    stamp = dt.datetime.now().strftime("%Y%m%d-%H%M%S")
    run_dir = LOG_ROOT / folder / f"{stamp}{suffix}"
    ...
    def from_mapping(cls, payload: dict[str, Any]) -> "AppConfig":
        ...
        return cls(
            ...
            switch=SwitchConfig(
                ...
                deploy_path=str(switch_cfg["deploy_path"]),
            ),
            prompts=PromptConfig(
...
def configure_nxp_hwpartition_if_needed(session: SerialSession, timeout: int) -> None:
    ...
    for line in output.splitlines():
        if line.strip().startswith("User Capacity:"):
            user_capacity_line = line.strip()
            break

    if "ENH WRREL" in user_capacity_line:
        info(f"NXP: mmc user partition already configured ({user_capacity_line}); skipping hwpartition command")
        return

    if not user_capacity_line:
        warn("NXP: mmc info did not include a User Capacity line; sending hwpartition command")
    else:
        info(f"NXP: mmc user partition is not ENH WRREL yet ({user_capacity_line}); sending hwpartition command")

    run_command(
        session,
        "mmc hwpartition user enh 0 82837504 complete",
        UBOOT_PROMPT_PATTERN,
        timeout,
        "NXP U-Boot: mmc hwpartition user enh 0 82837504 complete",
    )
...
def configure_switch_hwpartition_if_needed(
    session: SerialSession,
    prompt_pattern: re.Pattern[str],
    timeout: int,
) -> None:
    ...
    if "ENH WRREL" in user_capacity_line:
        info(f"Switch: mmc user partition already configured ({user_capacity_line}); skipping hwpartition command")
        return

    if not user_capacity_line:
        warn("Switch: mmc info did not include a User Capacity line; sending hwpartition command")
    else:
        info(f"Switch: mmc user partition is not ENH WRREL yet ({user_capacity_line}); sending hwpartition command")
    ...
    run_command(
        ...
        prompt_pattern,
        timeout,
        "Switch U-Boot: mmc hwpartition user enh 0 20709376 wrrel on complete",
    )

def configure_switch_hwpartition(config: AppConfig, args: argparse.Namespace, run_dir: pathlib.Path) -> None:
    ...
    with SerialSession(
        ...
        switch_prompt = detect_switch_uboot(
            ...
            max(config.timeouts.uboot_boot_seconds, config.timeouts.boot_interrupt_seconds),
        )
        configure_switch_hwpartition_if_needed(switch, switch_prompt, config.timeouts.prompt_wait_seconds)
...
def reset_nxp_from_uboot_to_emergency(session: SerialSession, config: AppConfig) -> None:
    ...
    info("NXP: sending CPLD board power-cycle command from U-Boot, waiting 65 seconds without UART reads, then checking YAML emergency prompt")
    session.clear_buffer()
    session.send_line("cpld w e1 1")
...
def upload_directory_from_windows_to_dut(config: AppConfig, local_path: pathlib.Path, label: str) -> tuple[str, int]:
    if paramiko is None:
        raise RuntimeError(f"paramiko is required to copy {label} with password authentication.")
    if not local_path.is_dir():
        raise FileNotFoundError(f"{label} directory not found on the Windows host: {local_path}")
    ...
    try:
        ...
        client.close()
    ok(f"Copied {label} to {config.dut.login}@{config.dut.final_ip}:{remote_root} ({uploaded_files} files)")
    return remote_root, uploaded_files

def upload_lsbb_utils_from_windows_to_dut(config: AppConfig) -> tuple[str, int]:
    return upload_directory_from_windows_to_dut(config, pathlib.Path(config.dut.utils_path), "LSBB_Utils")

def upload_switch_deploy_from_windows_to_dut(config: AppConfig) -> tuple[str, int]:
    return upload_directory_from_windows_to_dut(config, pathlib.Path(config.switch.deploy_path), "switch deploy files")
...
def transfer_switch_deploy_after_login(session: SerialSession, config: AppConfig) -> None:
    shell_prompt = ROOT_SHELL_PATTERN
    remote_root, _ = upload_switch_deploy_from_windows_to_dut(config)
    verify_output = run_command_capture(
        session,
        f"test -d {remote_root} && echo OK",
        shell_prompt,
        max(config.timeouts.prompt_wait_seconds, 60),
        "Verify switch deploy files exist on the DUT",
    )
    if "OK" not in verify_output:
        raise RuntimeError(f"Expected {remote_root} on the DUT after SFTP transfer, but verification did not return OK.")
...
def run_nxp_flow(config: AppConfig, provision: ProvisionArgs, args: argparse.Namespace, run_dir: pathlib.Path) -> int:
    ...
    try:
        with SerialSession(
            ...
            configure_nxp_hwpartition_if_needed(nxp, config.timeouts.prompt_wait_seconds)
            configure_switch_hwpartition(config, args, run_dir)

            if args.monitor_uart_seconds > 0:
                stage(2, "Power-cycle board after MAC save and monitor raw UART only")
                nxp.clear_buffer()
                info("NXP: sending CPLD board power-cycle command from U-Boot without emergency detection")
                nxp.send_line("cpld w e1 1")
                monitor_uart_only(nxp, args.monitor_uart_seconds)
                ok("UART monitor-only run completed")
                return 0

            stage(2, "Power-cycle board after MAC save and enter emergency mode using YAML prompt")
            ...
            stage(8, "Copy switch deploy files from Windows to the DUT")
            transfer_switch_deploy_after_login(nxp, config)
...
def main(argv: list[str] | None = None) -> int:
    ...
    run_dir = run_dir_for_dig_sn(args.dig_sn, "_nxp_deploy")
    try:
