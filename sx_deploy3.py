...
def run_windows_command_with_password(command: list[str], password: str, timeout: int, label: str) -> str:
    ...
    try:
        temp_dir = pathlib.Path(tempfile.mkdtemp(prefix="sx_ver3_askpass_"))
        ...
        env.setdefault("DISPLAY", "sx_ver3")
...
def wait_with_tqdm(seconds: int, label: str) -> None:
    try:
        ...
        raise sx4000_config.ConfigError("tqdm is required for sx_deploy3.py. Install it with: python -m pip install tqdm") from exc
...
def configure_sx_ver2_sequence(config: SxVer2Config, repo_root: pathlib.Path, skip_transfer: bool) -> None:
    nxp_log = sx4000_config.timestamped_log_path(repo_root, "sx-ver3-nxp")
    ...
    with sx4000_config.SerialSession("NXP", config.nxp, nxp_log, config.serial_open_seconds) as nxp:
        ...
        try:
            ...
            try:
                ...
                if not active_error:
                    raise
                sx4000_config.warn(f"NXP UART: failed during SX ver3 cleanup after error: {exc}")


def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="SX4000 deploy3 runner using script_setup.yaml.")
...
def run(args: argparse.Namespace, repo_root: pathlib.Path, config_path: pathlib.Path) -> int:
    try:
        sx4000_config.info("Starting SX Deployment ver3 stage.")
        ...
        if args.mode == "check":
            nxp_log = sx4000_config.timestamped_log_path(repo_root, "sx-ver3-nxp-check")
            with sx4000_config.SerialSession("NXP", config.nxp, nxp_log, config.serial_open_seconds) as nxp:
