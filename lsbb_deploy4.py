...
DEPLOYMENTS = {
    "FULL DEPLOYMENT": [["lscriptnxp3.py"], ["eth_deploy2.py"], ["sx_deploy3.py"]],
    "NXP DEPLOYMENT": [["lscriptnxp3.py"]],
    "SWITCH DEPLOYMENT": [["eth_deploy2.py"]],
...
def reset_deployment_statuses_after_wipe(serial: str) -> None:
    config = load_sql_config()
    with open_sql_connection() as connection:
        ensure_sql_tables(connection)
        ensure_deployment_row(connection, serial)
...
class DeployGui(tk.Tk):
    def __init__(self) -> None:
        super().__init__()
        self.title("LSBB Deployment ver4.0")
    ...
    def _build_ui(self) -> None:
        ...
        self.wipe_button = ttk.Button(bottom_frame, text="Wipe", command=self.wipe_serial)
        self.wipe_button.grid(row=0, column=1, sticky="e", padx=(10, 0), ipadx=12)

        exit_button = ttk.Button(bottom_frame, text="Exit", command=self.on_exit)
        exit_button.grid(row=0, column=2, sticky="e", padx=(10, 0), ipadx=12)
    ...
    def _set_buttons_enabled(self, enabled: bool) -> None:
        ...
        self.wipe_button.configure(state=state)
    ...
    def _refresh_buttons_from_db(self) -> None:
        ...
        self.save_sfp_checkbox.configure(state="normal")
        self.wipe_button.configure(state="normal" if serial else "disabled")
    ...
    def wipe_serial(self) -> None:
        ...
        serial = self.serial_var.get().strip()
        if not serial:
            messagebox.showwarning("Missing serial", "Enter a serial number before wiping deployment data.")
            return
        try:
            serial = normalize_serial(serial)
            self.serial_var.set(serial)
        ...
        command = [sys.executable, "lsbb_wipe.py"]
        self._clear_output()
        self._append_output(f"> {' '.join(command)}\n\n")
        self.status_var.set("Running wipe...")
        self._set_buttons_enabled(False)

        thread = threading.Thread(target=self._run_wipe, args=(command, serial), daemon=True)
        thread.start()

    def _run_wipe(self, command: list[str], serial: str) -> None:
        success = False
        try:
            self.output_queue.put("\nStarting lsbb_wipe.py\n")
            self.current_process = subprocess.Popen(command, cwd=REPO_ROOT, stdin=subprocess.DEVNULL)
            return_code = self.current_process.wait()
            if return_code != 0:
                self.output_queue.put(f"\nWipe script failed with exit code {return_code}. Deployment statuses were not changed.\n")
                return

            reset_deployment_statuses_after_wipe(serial)
            success = True
            self.output_queue.put(
                f"\nWipe script finished successfully.\n"
                f"Reset deployment statuses for {serial}.\n"
                f"Serial and MAC SQL rows were kept.\n"
            )
        except Exception as exc:
            self.output_queue.put(f"\nWipe failed: {exc}\n")
        finally:
            self.last_result = ("Wipe", success)
            self.output_queue.put("__DEPLOYMENT_DONE__")
